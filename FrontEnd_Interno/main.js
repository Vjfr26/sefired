...
import {
  ...
  ChevronDown, ChevronRight, Layers, FileSearch, FileText,
...
const NAV = [
  ...
  { id: 'catalogos',  label: 'Catálogos y Parámetros',  icon: 'book-open',       subs: [
    { id: 'cat-productos', label: 'Productos y Coberturas' },
    { id: 'cat-tipos',     label: 'Tipos, Marcas y Modelos' },
    { id: 'cat-tasas',     label: 'Tasas Base y UT' },
    { id: 'cat-params',    label: 'Parámetros del Sistema' },
    { id: 'cat-config',    label: 'Configuración General' },
  ]},
  { id: 'clientes',   label: 'Clientes y Vehículos',    icon: 'car',             subs: [
    { id: 'cli-cliente',   label: 'Registro de Cliente' },
    { id: 'cli-tomador',   label: 'Registro de Tomador' },
    { id: 'cli-conductor', label: 'Registro de Conductores' },
    { id: 'cli-vehiculo',  label: 'Registro de Vehículos' },
  ]},
  { id: 'cotizacion', label: 'Cotización / Solicitud',  icon: 'file-plus',       subs: [
    { id: 'cot-nueva',     label: 'Nueva Solicitud' },
    { id: 'cot-vehiculo',  label: 'Selección de Vehículo' },
    { id: 'cot-conductor', label: 'Selección de Conductor' },
    { id: 'cot-coberturas',label: 'Coberturas y Planes' },
    { id: 'cot-calculo',   label: 'Cálculo de Prima' },
  ]},
  { id: 'revision',   label: 'Revisión y Aprobación',   icon: 'clipboard-check', subs: [
    { id: 'rev-validacion',label: 'Validación de Datos' },
    { id: 'rev-coberturas',label: 'Revisión de Coberturas' },
    { id: 'rev-evaluacion',label: 'Evaluación de Solicitud' },
    { id: 'rev-decision',  label: 'Aprobación / Rechazo' },
  ]},
  { id: 'emision',    label: 'Emisión de Póliza',       icon: 'shield-check',    subs: [
    { id: 'emi-generacion',label: 'Generación de Póliza' },
    { id: 'emi-numero',    label: 'Asignación de Número' },
    { id: 'emi-planes',    label: 'Planes y Coberturas' },
    { id: 'emi-recibo',    label: 'Generación de Recibo' },
  ]},
  { id: 'facturacion',label: 'Facturación y Cobro',     icon: 'receipt',         subs: [
    { id: 'fac-factura',   label: 'Generación de Factura' },
    { id: 'fac-impuestos', label: 'Impuestos y Tasas' },
    { id: 'fac-pago',      label: 'Método de Pago' },
    { id: 'fac-registro',  label: 'Registro de Pago' },
  ]},
  { id: 'ventas',     label: 'Venta y Comisiones',      icon: 'trending-up',     subs: [
    { id: 'ven-venta',     label: 'Registro de Venta' },
    { id: 'ven-comisiones',label: 'Cálculo de Comisiones' },
    { id: 'ven-metas',     label: 'Metas y Producción' },
  ]},
  { id: 'reportes',   label: 'Reportes y Consultas',    icon: 'bar-chart-3',     subs: [
    { id: 'rep-ventas',    label: 'Reportes de Ventas' },
    { id: 'rep-polizas',   label: 'Pólizas Emitidas' },
    { id: 'rep-produccion',label: 'Producción por Vendedor' },
    { id: 'rep-comisiones',label: 'Comisiones' },
    { id: 'rep-tasas',     label: 'Tasa del Día e Históricos' },
  ]},
  { id: 'tasas',      label: 'Tasas del Día',           icon: 'dollar-sign',     subs: [
    { id: 'tas-registro',  label: 'Registro de Tasa BCV' },
    { id: 'tas-historico', label: 'Histórico de Tasas' },
    { id: 'tas-aplicacion',label: 'Aplicación Automática' },
  ]},
  { id: 'configuracion', label: 'Configuración',        icon: 'settings',        subs: [
    { id: 'conf-perfil',   label: 'Mi Perfil' },
    { id: 'conf-seguridad',label: 'Seguridad' },
    { id: 'conf-prefs',    label: 'Preferencias' },
    { id: 'conf-usuarios', label: 'Usuarios del Sistema' },
    { id: 'conf-roles',    label: 'Roles y Permisos' },
    { id: 'conf-empresa',  label: 'Empresa' },
    { id: 'conf-auditoria',label: 'Auditoría' },
    { id: 'conf-respaldo', label: 'Respaldos' },
  ]},
];
...
function tbl(cols, rows, footer = '') {
  ...
  const body = rows.map(r =>
    `<tr class="hover:bg-slate-50 transition-colors">${
      cols.map(c => `<td class="td-cell${c.r?' text-right':''}${c.m?' font-mono text-xs':''}">${r[c.k] ?? '—'}</td>`).join('')
    }</tr>`
  ).join('');
...
function viewHome() {
  ...
  return `
  <!-- Bienvenida -->
  <div class="card p-10 mb-8 text-center bg-gradient-to-br from-[#0f172a] to-[#1e3a5f] border-none">
    <div class="w-20 h-20 rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-3xl font-extrabold text-white mx-auto mb-5 shadow-2xl shadow-blue-500/40">VA</div>
    <h2 class="text-3xl font-extrabold text-white mb-1">Bienvenido, Victor</h2>
    <p class="text-sm text-blue-300 font-medium">Gerente Regional</p>
    <p class="text-xs text-slate-400 mt-2">${fecha} · ${hora}</p>
  </div>

  <!-- KPIs del día -->
  <div class="grid grid-cols-4 gap-5 mb-8">
    <div class="card p-5 border-l-4 border-l-blue-500">
      <p class="text-xs text-slate-500 mb-0.5">Tasa BCV Hoy</p>
      <p class="text-2xl font-bold text-blue-700">38.54 Bs/$</p>
      <p class="text-[11px] text-emerald-600 mt-1 font-medium">↑ +0.14% vs. ayer</p>
    </div>
    <div class="card p-5 border-l-4 border-l-amber-500">
      <p class="text-xs text-slate-500 mb-0.5">Solicitudes Pendientes</p>
      <p class="text-2xl font-bold text-slate-800">5</p>
      <p class="text-[11px] text-amber-600 mt-1 font-medium">Esperando revisión</p>
    </div>
    <div class="card p-5 border-l-4 border-l-emerald-500">
      <p class="text-xs text-slate-500 mb-0.5">Pólizas Emitidas Hoy</p>
      <p class="text-2xl font-bold text-slate-800">3</p>
      <p class="text-[11px] text-emerald-600 mt-1 font-medium">SEF-2026-VEH-00846/7/8</p>
    </div>
    <div class="card p-5 border-l-4 border-l-rose-500">
      <p class="text-xs text-slate-500 mb-0.5">Cobros Pendientes</p>
      <p class="text-2xl font-bold text-slate-800">7</p>
      <p class="text-[11px] text-rose-600 mt-1 font-medium">${usd(4358.90)} por cobrar</p>
    </div>
  </div>
  ...
  <div class="grid grid-cols-3 gap-6">
    <div class="col-span-2 card p-6">
      <h4 class="font-bold text-slate-800 text-sm mb-4">Accesos Rápidos</h4>
      <div class="grid grid-cols-3 gap-3">
        ${[
          {icon:'file-plus',    label:'Nueva Solicitud',       view:'cot-nueva',     color:'blue'},
          {icon:'dollar-sign',  label:'Registrar Tasa BCV',    view:'tas-registro',  color:'indigo'},
          {icon:'shield-check', label:'Emitir Póliza',         view:'emi-generacion',color:'emerald'},
          {icon:'receipt',      label:'Registrar Cobro',       view:'fac-registro',  color:'amber'},
          {icon:'bar-chart-3',  label:'Reportes de Ventas',    view:'rep-ventas',    color:'slate'},
          {icon:'users',        label:'Gestión de Clientes',   view:'cli-cliente',   color:'purple'},
        ].map(a => `
          <button onclick="navigateTo('${a.view}')" class="card p-4 text-center hover:border-${a.color}-300 hover:shadow-md transition cursor-pointer group border-2 border-transparent">
            <div class="w-10 h-10 rounded-xl bg-${a.color}-50 group-hover:bg-${a.color}-100 flex items-center justify-center mx-auto mb-2 transition">
              <i data-lucide="${a.icon}" class="w-5 h-5 text-${a.color}-600"></i>
            </div>
            <p class="text-xs font-semibold text-slate-700 leading-tight">${a.label}</p>
          </button>`).join('')}
      </div>
    </div>

    <div class="card p-6">
      <h4 class="font-bold text-slate-800 text-sm mb-4">Pendientes del Día</h4>
      <div class="space-y-3">
        ${[
          {ico:'alert-triangle', col:'rose',    txt:'5 solicitudes sin revisar',     sub:'Revisión y aprobación',  view:'rev-decision'},
          {ico:'receipt',        col:'amber',   txt:'7 cobros pendientes',           sub:'Facturación y cobro',    view:'fac-registro'},
          {ico:'clock',          col:'amber',   txt:'12 pólizas vencen esta semana', sub:'Reportes · Pólizas',     view:'rep-polizas'},
          {ico:'dollar-sign',    col:'blue',    txt:'Tasa BCV registrada',           sub:'38.54 Bs/$ — 03/05/2026',view:'tas-registro'},
          {ico:'trending-up',    col:'emerald', txt:'3 comisiones por pagar',        sub:'Venta y comisiones',     view:'ven-comisiones'},
        ].map(t => `
          <button onclick="navigateTo('${t.view}')" class="w-full flex items-start gap-3 p-2.5 rounded-xl hover:bg-slate-50 transition text-left cursor-pointer">
            <div class="p-1.5 bg-${t.col}-100 rounded-lg text-${t.col}-600 shrink-0 mt-0.5">
              <i data-lucide="${t.ico}" class="w-3.5 h-3.5"></i>
            </div>
            <div>
              <p class="text-xs font-semibold text-slate-800">${t.txt}</p>
              <p class="text-[10px] text-slate-400 mt-0.5">${t.sub}</p>
            </div>
          </button>`).join('')}
      </div>
    </div>
  ...
  return `<div class="grid grid-cols-3 gap-6">
    <div class="card p-6 col-span-2">
      ...
      <div class="flex items-center gap-5 pb-5 mb-5 border-b border-slate-100">
        <div class="w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-xl font-extrabold text-white shrink-0">VA</div>
        <div>
          <p class="font-bold text-slate-800">Victor Admin</p>
          <p class="text-sm text-slate-500">Gerente Regional · Sefired C.A.</p>
...
function confUsuarios() {
  ...
  tbl([{l:'Usuario',k:'usr',m:true},{l:'Nombre',k:'nom'},{l:'Rol',k:'rol'},{l:'Email',k:'email'},{l:'Último Acceso',k:'ult'},{l:'Estado',k:'est'},{l:'',k:'acc'}],
    usuarios.map(u => ({...u, acc: `<div class="flex gap-1 justify-center">
...
function renderSidebarNav() {
  ...
  nav.innerHTML = NAV.map(g => {
    if (g.direct) {
      ...
      return `
    <div>
      <button class="group-btn ${isActive ? 'group-btn-active' : ''}" data-direct="${g.viewId}">
    ...
    return `
    <div>
      <button class="group-btn ${isOpen ? 'open' : ''}" data-group="${g.id}">
        <i data-lucide="${g.icon}" class="w-4 h-4 shrink-0"></i>
        <span class="flex-1 leading-tight text-left">${g.label}</span>
        <i data-lucide="chevron-down" class="chevron ${isOpen ? 'open' : ''} w-3.5 h-3.5"></i>
      </button>
      <div class="sub-items ${isOpen ? 'open' : ''}">
        ${g.subs.map(s => `
          <button class="sub-btn ${s.id === activeView ? 'active' : ''}" data-view="${s.id}">
  ...
  // Direct item click
  nav.querySelectorAll('[data-direct]').forEach(btn => {
    btn.addEventListener('click', () => navigateTo(btn.dataset.direct));
  });

  // Group toggle
  nav.querySelectorAll('.group-btn[data-group]').forEach(btn => {
...
const ALL_ICONS = {
  ...
  Layers, FileSearch, FileText, Users, UserCog, Wrench, Tag,
  ...
  LogOut, Bell, Search, Shield, UserCheck, Building2,
  CheckCircle, Info, Printer, Send, CreditCard,
  User, Home, KeyRound, Monitor, Database, Activity, History, UserX, Power,
};

function navigateTo(viewId) {
  const directItem = NAV.find(g => g.direct && g.viewId === viewId);
  const group = directItem ? null : NAV.find(g => g.subs?.some(s => s.id === viewId));

  if (!directItem && !group) return;

  activeView = viewId;
  if (!directItem) openGroup = group.id;

  renderSidebarNav();

  if (directItem) {
    document.getElementById('page-title').textContent    = directItem.label;
    document.getElementById('page-subtitle').textContent = 'Panel Interno · Sefired';
  } else {
    const sub = group.subs.find(s => s.id === viewId);
    document.getElementById('page-title').textContent    = sub?.label  || '';
    document.getElementById('page-subtitle').textContent = group.label || '';
  }

  const area = document.getElementById('content-area');
  if (area) {
    const fn = VIEWS[viewId];
    area.innerHTML = fn ? fn() : `<div class="card p-8 text-center text-slate-400">Vista en construcción</div>`;
    createIcons({ icons: ALL_ICONS });
    setupTabListeners();
  }
...
function setupUserMenu() {
  ...
  document.getElementById('menu-go-perfil')?.addEventListener('click', () => {
    dropdown?.classList.add('hidden');
    chevron?.classList.remove('rotate-180');
    navigateTo('conf-perfil');
  });

  document.getElementById('menu-go-config')?.addEventListener('click', () => {
    dropdown?.classList.add('hidden');
    chevron?.classList.remove('rotate-180');
    navigateTo('conf-seguridad');
  });

...
export async function fetchSuperintendencia(params = {}) {
  const qs = new URLSearchParams(Object.fromEntries(Object.entries(params).filter(([, v]) => v != null && v !== '')))
  const res = await fetch(`${API_BASE_URL}/api/reports/superintendencia?${qs}`, { headers: getAuthHeaders() })
  if (!res.ok) throw new Error('Error al cargar reporte de superintendencia')
  return res.json()
}
...
export async function exportSuperintendencia(params = {}) {
  const token = localStorage.getItem('auth_token')
  const res = await fetch(`${API_BASE_URL}/api/reports/superintendencia/exportar`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Accept': '*/*', 'Authorization': `Bearer ${token}` },
    body: JSON.stringify(params)
  })
  if (!res.ok) throw new Error('Error al exportar superintendencia')
  return res.blob()
}

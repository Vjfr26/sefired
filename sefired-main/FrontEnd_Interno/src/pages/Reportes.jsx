...
import { TrendingUp, Shield, Building2, Users, RefreshCw, Search, Download, Check, Calendar, Loader2, Play, CheckCircle2, Car, Filter, ChevronDown, ChevronUp, AlertTriangle, X, Clock, Bookmark, Trash2 } from 'lucide-react'
import {
...
function TabSuperintendencia() {
  ...
  const [fechaInicio, setFechaInicio] = useState(start)
  const [fechaFin, setFechaFin] = useState(today)
  const [stats, setStats] = useState({ polizas_emitidas: 0, prima_total: 0, rc_obligatoria: 0, cancelaciones: 0 })
  ...
  const loadData = async () => {
    setLoading(true)
    try {
      const data = await fetchSuperintendencia({ fecha_inicio: fechaInicio, fecha_fin: fechaFin })
      setStats(data.stats || { polizas_emitidas: 0, prima_total: 0, rc_obligatoria: 0, cancelaciones: 0 })
      setRows(data.rows || [])
    } catch (err) {
      showToast('Error al cargar reporte de superintendencia', 'error')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleExport = async () => {
    setExporting(true)
    showToast('Exportando reporte SUDEASEG…', 'info')
    try {
      const blob = await exportSuperintendencia({
        fecha_inicio: fechaInicio,
        fecha_fin: fechaFin
      })
      downloadBlob(blob, `reporte_sudeaseg_${new Date().toISOString().slice(0, 10)}.xlsx`)
      showToast('Reporte SUDEASEG exportado correctamente', 'success')
    } catch (err) {
      showToast('Error al exportar reporte SUDEASEG', 'error')
    } finally {
      setExporting(false)
    }
  }
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-2 xl:grid-cols-4 gap-4 mb-5">
            {[
              { label: 'Pólizas Emitidas', val: stats.polizas_emitidas, sub: formatDateRange(fechaInicio, fechaFin), cls: 'border-t-blue-500',    vcls: 'text-slate-800' },
              { label: 'Prima Total',      val: usd(stats.prima_total), sub: 'USD',        cls: 'border-t-emerald-500', vcls: 'text-emerald-700' },
              { label: 'RC Obligatoria',   val: stats.rc_obligatoria,   sub: 'pólizas',    cls: 'border-t-amber-500',   vcls: 'text-amber-700' },
              { label: 'Cancelaciones',    val: stats.cancelaciones,    sub: formatDateRange(fechaInicio, fechaFin), cls: 'border-t-slate-400',   vcls: 'text-slate-700' },
            ].map(c => (
                ...
                <div className="flex-1">
                  ...
                  {sched.ultimo_envio && (
                    <p className="text-xs text-blue-600 font-medium mt-1">
                      Último envío: {new Date(sched.ultimo_envio).toLocaleString()}
                    </p>
                  )}
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card p-6">
          ...
          {loadingSchedules ? (
            ...
            <div className="space-y-3">
              {schedules.map(sched => (
                <div key={sched.id} className="flex flex-wrap items-center justify-between p-3.5 rounded-xl border border-slate-100 bg-slate-50/50 gap-4">
                  <div className="flex-1">
                    <h4 className="text-sm font-semibold text-slate-700">{sched.nombre}</h4>
                    <p className="text-xs text-slate-400 mt-0.5 capitalize">Período: {sched.frecuencia}</p>
                    {sched.ultimo_envio && (
                      <p className="text-xs text-blue-600 font-medium mt-1">
                        Última corrida: {new Date(sched.ultimo_envio).toLocaleString()}
                      </p>
                    )}
                  </div>
          ...
          {loadingHistory ? (
            ...
            <DataTable
              ...
              rows={history.map(item => ({
                ...
                fecha: new Date(item.fecha_generacion).toLocaleString(),
...
function TabClientesMetrics() {
  ...
  const loadClientDetail = async (clientId) => {
    setLoadingDetail(true)
    try {
      const data = await fetchClientesReport({ fecha_inicio: fechaInicio, fecha_fin: fechaFin, cliente_id: clientId })

...
@section('title', 'Recordatorio de Cuota — LA VENEZOLANA DE SEGUROS Y VIDA C.A.')
...
<h1 style="margin:0 0 8px;font-size:22px;font-weight:800;color:#1e293b;text-align:center;">
  @if($diasRestantes <= 0)
    Su cuota N° {{ $numeroCuota }} está vencida
  @elseif($diasRestantes === 1)
    Su cuota N° {{ $numeroCuota }} vence hoy
  @else
    Su cuota N° {{ $numeroCuota }} vence en {{ $diasRestantes }} días
  @endif
</h1>
...
<p style="margin:0 0 24px;font-size:14px;color:#64748b;text-align:center;line-height:1.6;">
  ...
  @if($diasRestantes <= 0)
    la cuota N° <strong>{{ $numeroCuota }}</strong> de su póliza <strong>{{ $nroContrato }}</strong> venció el <strong>{{ $fechaVencimiento }}</strong>. Por favor, regularice su pago para mantener la cobertura al día.
  @else
    le recordamos que la cuota N° <strong>{{ $numeroCuota }}</strong> de su póliza <strong>{{ $nroContrato }}</strong> vence el <strong>{{ $fechaVencimiento }}</strong>.
  @endif
</p>
...
<table width="100%" cellpadding="0" cellspacing="0"
  ...
  <tr><td style="padding:20px 24px;">
    <table width="100%" cellpadding="0" cellspacing="0">
      @foreach([
        ['Póliza',       $nroContrato],
        ['Cuota',        'N° ' . $numeroCuota . ' de 12'],
        ['Vencimiento',  $fechaVencimiento],
        ['Monto a pagar', $simboloNativo . $montoCuota . ' ' . $monedaNativa],
      ] as $row)
      <tr>
...
<table width="100%" cellpadding="0" cellspacing="0">
  <tr><td align="center">
    <a href="https://wa.me/584148299562?text=Hola%2C%20deseo%20pagar%20mi%20cuota%20mensual."
